    ...
    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-lg p-3 text-xs">
      ...
      {payload.map((entry, idx) => (
        <div key={idx} className="flex items-center gap-2">
          ...
          <span className="font-medium">{typeof entry.value === 'number' && entry.value >= 1000 ? formatMoneyCompact(entry.value) : entry.value}</span>
        </div>
      ))}
    </div>
  );
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button variant="secondary" iconLeft={<Download size={16} />} size="sm">
              {t('analytics.dashboard.export')}
            </Button>
            <Button variant="secondary" iconLeft={<Printer size={16} />} size="sm">
        ...
        <WidgetCard title={t('analytics.dashboard.financialMetrics')}>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={financialBars} barGap={4}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v / 1_000_000).toFixed(0)}${t('analytics.dashboard.millionShort')}`} />
              <Tooltip content={<CustomTooltip />} />
        ...
        <WidgetCard title={t('analytics.dashboard.procurementSpend')}>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={procurementSpend} layout="vertical" barGap={2}>
              ...
              <XAxis type="number" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v / 1_000_000).toFixed(0)}${t('analytics.dashboard.millionShort')}`} />

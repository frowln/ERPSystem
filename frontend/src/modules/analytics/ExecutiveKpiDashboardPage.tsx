...
export default function ExecutiveKpiDashboardPage() {
  ...
  const projectColumns: ColumnDef<ProjectHealth>[] = [
    ...
    },
    {
      ...
      header: 'CPI',
      cell: ({ row }) => (
        <span className={row.original.cpi < 0.9 ? 'text-red-600 font-semibold' : row.original.cpi < 1.0 ? 'text-yellow-600' : 'text-green-600'}>
    ...
    },
    {
      ...
      header: 'SPI',
      cell: ({ row }) => (
        <span className={row.original.spi < 0.9 ? 'text-red-600 font-semibold' : row.original.spi < 1.0 ? 'text-yellow-600' : 'text-green-600'}>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
            <h3 className="text-lg font-semibold mb-4">{t('executiveKpi.projectHealth')}</h3>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{dashboard?.projectHealth?.filter(p => p.healthStatus === 'GREEN').length ?? 0}</div>
                <div className="text-sm text-green-700">{t('executiveKpi.healthGreen')}</div>
              </div>
              <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                <div className="text-2xl font-bold text-yellow-600">{dashboard?.projectHealth?.filter(p => p.healthStatus === 'YELLOW').length ?? 0}</div>
                <div className="text-sm text-yellow-700">{t('executiveKpi.healthYellow')}</div>
              </div>
              <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
                <div className="text-2xl font-bold text-red-600">{dashboard?.projectHealth?.filter(p => p.healthStatus === 'RED').length ?? 0}</div>
                <div className="text-sm text-red-700">{t('executiveKpi.healthRed')}</div>
              </div>
            </div>
          </div>
        </>
      ...
      {activeTab === 'safety' && (
        <div className="space-y-6">
          ...
          {sm?.severityBreakdown && Object.keys(sm.severityBreakdown).length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
              <h3 className="text-lg font-semibold mb-4">{t('executiveKpi.severityBreakdown')}</h3>

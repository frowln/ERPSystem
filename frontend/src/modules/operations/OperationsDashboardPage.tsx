...
const OperationsDashboardPage: React.FC = () => {
  ...
  const recentActivities: RecentActivity[] = [
    ...(dailyLogsData?.content ?? []).map((dl) => ({
      id: dl.id,
      type: 'DAILY_LOG' as const,
      title: `${t('operations.dashboard.dailyLog')} ${formatDate(dl.logDate)}`,
      status: dl.status,
  ...
  for (const r of resourceSchedules) {
    const key = r.projectName ?? r.projectId;
    const existing = resourceMap.get(key) ?? { project: key, workers: 0, equipment: 0, activeOrders: 0 };
    if (r.resourceType === 'LABOR') existing.workers += r.quantity;
    else if (r.resourceType === 'EQUIPMENT') existing.equipment += r.quantity;
  ...
  const activeOrders = workOrdersData?.content?.filter((wo) => wo.status === 'IN_PROGRESS').length ?? 0;
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard icon={<Users size={18} />} label={t('operations.dashboard.workersOnSite')} value={totalWorkers} trend={{ direction: 'up', value: t('operations.dashboard.today') }} />
        <MetricCard icon={<Truck size={18} />} label={t('operations.dashboard.equipmentUnits')} value={totalEquipment} />
        <MetricCard icon={<Wrench size={18} />} label={t('operations.dashboard.activeOrders')} value={activeOrders} />
        <MetricCard icon={<TrendingUp size={18} />} label={t('operations.dashboard.averageReadiness')} value={`${workOrdersData?.content?.length ? Math.round((workOrdersData.content.filter(w => w.status === 'COMPLETED').length / workOrdersData.content.length) * 100) : 0}%`} />
      </div>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6 mt-6">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-danger-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-danger-800">{t('operations.dashboard.overdueOrders')}</p>
                  <p className="text-xs text-danger-600 mt-0.5">{t('operations.dashboard.requireUrgentAttention')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 bg-warning-50 rounded-lg">
                <Calendar size={16} className="text-warning-600 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-sm font-medium text-warning-800">{t('operations.dashboard.unapprovedLog')}</p>
                  <p className="text-xs text-warning-600 mt-0.5">{t('operations.dashboard.awaitingApproval')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-3 bg-success-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-success-800">{t('operations.dashboard.completedOrders')}</p>
                  <p className="text-xs text-success-600 mt-0.5">{t('operations.dashboard.thisMonth')}</p>
                </div>
              </div>
            </div>
          </div>

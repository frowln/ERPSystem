...
const getDefaultColumns = (): BoardColumn[] => [
  { id: 'SCHEDULED', title: t('fleet.maintenanceBoard.colScheduled'), color: 'bg-blue-500', headerBg: 'bg-blue-50', collapsed: false },
  { id: 'IN_PROGRESS', title: t('fleet.maintenanceBoard.colInProgress'), color: 'bg-yellow-500', headerBg: 'bg-yellow-50', collapsed: false },
  { id: 'COMPLETED', title: t('fleet.maintenanceBoard.colCompleted'), color: 'bg-green-500', headerBg: 'bg-green-50', collapsed: false },
];
...
const FleetMaintenanceBoardPage: React.FC = () => {
  ...
  const [items, setItems] = useState<FmCard[]>([]);
  ...
  const onDrop = useCallback((e: React.DragEvent, col: FmStatus) => { e.preventDefault(); if (!draggedId) return; setItems((p) => p.map((i) => (i.id === draggedId ? { ...i, status: col } : i))); setDraggedId(null); setDragOverCol(null); }, [draggedId]);
            ...
            <div key={col.id} className={cn('flex flex-col min-w-[280px] w-[280px] rounded-xl border transition-all duration-200', isOver ? 'border-primary-400 bg-primary-50/30 shadow-md' : 'border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50', col.collapsed && 'min-w-[48px] w-[48px]')} onDragOver={(e) => { e.preventDefault(); setDragOverCol(col.id); }} onDrop={(e) => onDrop(e, col.id)}>
              ...
              {!col.collapsed && (
                <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[100px]">
                  {colItems.length === 0 ? (<div className="flex flex-col items-center justify-center py-8 text-center"><p className="text-xs text-neutral-400">{t('fleet.maintenanceBoard.emptyColumn')}</p><p className="text-[10px] text-neutral-300 mt-0.5">{t('fleet.maintenanceBoard.dragHint')}</p></div>) : colItems.map((item) => (
                    <div key={item.id} draggable onDragStart={(e) => onDragStart(e, item.id)} onClick={() => navigate('/fleet/maintenance')} className={cn('bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-700 p-3 cursor-pointer hover:shadow-md hover:border-neutral-300 dark:hover:border-neutral-600 transition-all', draggedId === item.id && 'opacity-50 shadow-lg')}>
                      ...
                      <p className="text-[10px] text-neutral-400 mb-1">{item.vehiclePlate} | {item.mileage.toLocaleString('ru-RU')} {t('fleet.maintenanceBoard.kmSuffix')}</p>
                      <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-600 mb-2">{item.serviceType}</span>

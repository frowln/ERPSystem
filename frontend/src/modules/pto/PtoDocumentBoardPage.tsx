...
const PtoDocumentBoardPage: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in" onDragEnd={onDragEnd}>
      <PageHeader title="Документы ПТО - Доска" subtitle={`${items.length} документов`} breadcrumbs={[{ label: 'Главная', href: '/' }, { label: 'ПТО', href: '/pto/documents' }, { label: 'Доска' }]} actions={<div className="flex items-center gap-2"><Button variant="secondary" size="sm" iconLeft={<Filter size={14} />} onClick={() => setShowFilters(!showFilters)} className={hasFilters ? 'border-primary-300 text-primary-600' : ''}>Фильтры</Button><Button iconLeft={<Plus size={16} />}>Новый документ</Button></div>} />
      {showFilters && (
            ...
            <div key={col.id} className={cn('flex flex-col min-w-[280px] w-[280px] rounded-xl border transition-all duration-200', isOver ? 'border-primary-400 bg-primary-50/30 shadow-md' : 'border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50', col.collapsed && 'min-w-[48px] w-[48px]')} onDragOver={(e) => { e.preventDefault(); setDragOverCol(col.id); }} onDrop={(e) => onDrop(e, col.id)}>
              ...
              {!col.collapsed && (
                <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[100px]">
                  {colItems.length === 0 ? (<div className="flex flex-col items-center justify-center py-8 text-center"><p className="text-xs text-neutral-400">Нет документов</p><p className="text-[10px] text-neutral-300 mt-0.5">Перетащите карточку сюда</p></div>) : colItems.map((item) => (

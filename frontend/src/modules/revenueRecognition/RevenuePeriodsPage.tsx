...
const RevenuePeriodsPage: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Периоды признания выручки"
        subtitle="Сводные данные по периодам для всех контрактов"
        breadcrumbs={[
          { label: 'Главная', href: '/' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard icon={<CalendarDays size={18} />} label="Записей в текущем кв." value={filtered.filter((s: PeriodSummary) => s.period === '2026-Q1').length} />
        <MetricCard icon={<DollarSign size={18} />} label="Выручка (тек. кв.)" value={formatMoneyCompact(totals.totalRevenue)} />
        <MetricCard icon={<TrendingUp size={18} />} label="Прибыль (тек. кв.)" value={formatMoneyCompact(totals.totalProfit)} />
        <MetricCard icon={<BarChart3 size={18} />} label="Корректировки" value={formatMoney(totals.totalAdj)} />
      </div>

      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-xs">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" />
          <Input placeholder="Поиск по контракту..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
        <Select
      ...
      <DataTable<PeriodSummary>
        ...
        emptyDescription="Данные появятся после первого признания выручки"
      />
    </div>
  );
};

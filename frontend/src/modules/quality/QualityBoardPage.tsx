...
interface BoardColumn { id: QcStatus; title: string; color: string; headerBg: string; collapsed: boolean; }

const getDefaultColumns = (): BoardColumn[] => [
  { id: 'PENDING', title: t('quality.board.colPending'), color: 'bg-neutral-400', headerBg: 'bg-neutral-50 dark:bg-neutral-800', collapsed: false },
  { id: 'IN_PROGRESS', title: t('quality.board.colInProgress'), color: 'bg-yellow-500', headerBg: 'bg-yellow-50', collapsed: false },
  { id: 'PASSED', title: t('quality.board.colPassed'), color: 'bg-green-500', headerBg: 'bg-green-50', collapsed: false },
  { id: 'FAILED', title: t('quality.board.colFailed'), color: 'bg-red-500', headerBg: 'bg-red-50', collapsed: false },
];

const getPriorityLabels = (): Record<string, string> => ({ low: t('quality.board.priorityLow'), normal: t('quality.board.priorityNormal'), high: t('quality.board.priorityHigh'), critical: t('quality.board.priorityCritical') });
...
const QualityBoardPage: React.FC = () => {
  ...
  const [dragOverCol, setDragOverCol] = useState<QcStatus | null>(null);

  const filtered = useMemo(() => {
    let r = items;
    if (filterStatus) r = r.filter((i) => i.status === filterStatus);
    if (searchQuery) { const q = searchQuery.toLowerCase(); r = r.filter((i) => i.title.toLowerCase().includes(q) || i.code.toLowerCase().includes(q) || i.location.toLowerCase().includes(q)); }
    return r;
  }, [items, filterStatus, searchQuery]);

  const byColumn = useMemo(() => { const m: Record<string, QcCard[]> = {}; for (const c of columns) m[c.id] = filtered.filter((i) => i.status === c.id); return m; }, [filtered, columns]);
  ...
  const onDrop = useCallback((e: React.DragEvent, col: QcStatus) => { e.preventDefault(); if (!draggedId) return; setItems((p) => p.map((i) => (i.id === draggedId ? { ...i, status: col } : i))); setDraggedId(null); setDragOverCol(null); }, [draggedId]);
  const onDragEnd = useCallback(() => { setDraggedId(null); setDragOverCol(null); }, []);
  const toggleCol = useCallback((id: QcStatus) => { setColumns((p) => p.map((c) => (c.id === id ? { ...c, collapsed: !c.collapsed } : c))); }, []);

  const priorityLabels = getPriorityLabels();

  return (
    <div className="animate-fade-in" onDragEnd={onDragEnd}>
      <PageHeader title={t('quality.board.title')} subtitle={t('quality.board.subtitle', { count: String(items.length) })} breadcrumbs={[{ label: t('quality.board.breadcrumbHome'), href: '/' }, { label: t('quality.board.breadcrumbQuality'), href: '/quality' }, { label: t('quality.board.breadcrumbBoard') }]} actions={<div className="flex items-center gap-2"><Button variant="secondary" size="sm" iconLeft={<Filter size={14} />} onClick={() => setShowFilters(!showFilters)} className={hasFilters ? 'border-primary-300 text-primary-600' : ''}>{t('quality.board.btnFilters')}</Button><Button iconLeft={<Plus size={16} />} onClick={() => navigate('/quality/new')}>{t('quality.board.btnNewCheck')}</Button></div>} />
      {showFilters && (
        <div className="flex items-center gap-3 mb-4 p-3 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 animate-fade-in">
          <div className="relative flex-1 max-w-xs"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" /><Input placeholder={t('quality.board.searchPlaceholder')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" /></div>
          <Select options={[{ value: '', label: t('quality.board.filterAllStatuses') }, ...columns.map((c) => ({ value: c.id, label: c.title }))]} value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-48" />
          {hasFilters && <Button variant="ghost" size="sm" iconLeft={<X size={14} />} onClick={() => { setSearchQuery(''); setFilterStatus(''); }}>{t('quality.board.btnReset')}</Button>}
        </div>
      )}
            ...
            <div key={col.id} className={cn('flex flex-col min-w-[280px] w-[280px] rounded-xl border transition-all duration-200', isOver ? 'border-primary-400 bg-primary-50/30 shadow-md' : 'border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50', col.collapsed && 'min-w-[48px] w-[48px]')} onDragOver={(e) => { e.preventDefault(); setDragOverCol(col.id); }} onDrop={(e) => onDrop(e, col.id)}>
              ...
              {!col.collapsed && (
                <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[100px]">
                  {colItems.length === 0 ? (<div className="flex flex-col items-center justify-center py-8 text-center"><p className="text-xs text-neutral-400">{t('quality.board.emptyColumn')}</p><p className="text-[10px] text-neutral-300 mt-0.5">{t('quality.board.emptyColumnHint')}</p></div>) : colItems.map((item) => (
                    <div key={item.id} draggable onDragStart={(e) => onDragStart(e, item.id)} onClick={() => navigate(`/quality/${item.id}`)} className={cn('bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-700 p-3 cursor-pointer hover:shadow-md hover:border-neutral-300 dark:hover:border-neutral-600 transition-all', draggedId === item.id && 'opacity-50 shadow-lg')}>
                      ...
                      <div className="flex items-center justify-between">
                        {item.inspectorName ? (<div className="flex items-center gap-1.5"><div className="w-5 h-5 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center text-[10px] font-bold">{item.inspectorName.charAt(0)}</div><span className="text-[10px] text-neutral-500 dark:text-neutral-400 truncate max-w-[100px]">{item.inspectorName}</span></div>) : <div />}
                        <span className="text-[10px] text-neutral-400">{item.checkDate}</span>
                      </div>
                    </div>
                  ))}
                </div>
...
export default QualityBoardPage;

...
export default function SnapshotPanel({ budgetId, onCompare }: SnapshotPanelProps) {
  ...
  const createMutation = useMutation({
    mutationFn: () => financeApi.createSnapshot(budgetId, { name, notes: notes || undefined }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['budget-snapshots', budgetId] });
      setCreating(false);
      setName('');
      setNotes('');
      toast.success(t('fm.snapshot.create'));
    },
  });
        ...
        <div className="p-4 border-b border-neutral-200 dark:border-neutral-700 space-y-3">
          <div>
            <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">
              {t('fm.snapshot.nameLabel')}
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={t('fm.snapshot.namePlaceholder')}
          ...
          <div>
            <label className="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">
              {t('fm.snapshot.notesLabel')}
            </label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={t('fm.snapshot.notesPlaceholder')}
              rows={2}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!isLoading && items.length === 0 && (
          <div className="p-4 text-center">
            ...
            <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('fm.snapshot.noSnapshots')}</p>
            <p className="text-xs text-neutral-400 dark:text-neutral-500">{t('fm.snapshot.noSnapshotsDesc')}</p>
          </div>
        )}
          ...
          >
            ...
            <div className="flex gap-3 mt-1 text-xs">
              <span className="text-neutral-500 dark:text-neutral-400">
                {t('fm.kpiCostPrice')}: {fmt(snap.totalCost)}
              </span>
              <span className={snap.totalMargin >= 0 ? 'text-green-600' : 'text-red-600'}>
                {t('fm.kpiMargin')}: {fmt(snap.totalMargin)}
              </span>
            </div>
          </div>
        ))}

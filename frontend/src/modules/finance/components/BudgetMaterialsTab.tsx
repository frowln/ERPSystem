...
const BudgetMaterialsTab: React.FC<BudgetMaterialsTabProps> = ({ items, sections, onEditItem }) => {
  const materialItems = useMemo(
    () => items.filter((i) => !i.section && i.itemType === 'MATERIALS'),
    [items],
  );
  ...
  }, [materialItems, sections]);

  const totals = useMemo(
    () => ({
      costTotal: materialItems.reduce((s, i) => s + (i.costPrice || 0) * (i.quantity || 0), 0),
      estimateTotal: materialItems.reduce((s, i) => s + (i.estimatePrice || 0) * (i.quantity || 0), 0),
      customerTotal: materialItems.reduce((s, i) => s + (i.customerPrice || 0) * (i.quantity || 0), 0),
    }),
    [materialItems],
  );
  ...
  if (materialItems.length === 0) {
    return (
      <div className="text-center py-12 text-neutral-500 dark:text-neutral-400">
        {t('finance.budgetView.noMaterials')}
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-200 dark:border-primary-800 px-4 py-3 flex items-center justify-between">
        <span className="font-semibold text-neutral-900 dark:text-neutral-100">
          {t('finance.budgetView.totalMaterials')}
        </span>
...
export default BudgetMaterialsTab;

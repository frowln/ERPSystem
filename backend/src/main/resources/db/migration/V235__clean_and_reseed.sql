-- =============================================================================
-- V235: Clean ALL business data, keep only admin account, reseed demo project
--       with fully linked financial chain (invoices→budget, price sources, etc.)
-- =============================================================================
...
DECLARE
    ...
    -- Contract types
    ct_general UUID    := 'c0000001-0000-0000-0000-000000000001';
    ct_subcontract UUID := 'c0000001-0000-0000-0000-000000000002';
    ct_supply UUID     := 'c0000001-0000-0000-0000-000000000003';
    ct_design UUID     := 'c0000001-0000-0000-0000-000000000004';
    ct_service UUID    := 'c0000001-0000-0000-0000-000000000005';
...
BEGIN
    -- Find admin user and their organization
    SELECT id, organization_id INTO admin_id, org_id FROM users WHERE email = 'admin@privod.ru' LIMIT 1;
    IF admin_id IS NULL THEN
        SELECT id, organization_id INTO admin_id, org_id FROM users WHERE email = 'admin@privod.com' LIMIT 1;
    END IF;
    IF admin_id IS NULL THEN
        SELECT id, organization_id INTO admin_id, org_id FROM users LIMIT 1;
    END IF;
    ...
    -- =========================================================================
    -- PHASE 1: DELETE ALL BUSINESS DATA (global, not filtered by project)
    -- Use TRUNCATE ... CASCADE and guard by to_regclass for schema compatibility.
    -- =========================================================================
    FOREACH cleanup_table IN ARRAY ARRAY[
        'competitive_list_entries',
        'competitive_lists',
        'commercial_proposal_items',
        'commercial_proposals',
        'contract_budget_items',
        'ks3_ks2_links',
        'ks2_lines',
        'ks3_documents',
        'ks2_documents',
    ...
    LOOP
        IF to_regclass(cleanup_table) IS NOT NULL THEN
            EXECUTE format('TRUNCATE TABLE %I RESTART IDENTITY CASCADE', cleanup_table);
        END IF;
    END LOOP;

    -- =========================================================================
    -- PHASE 2: INSERT DEMO DATA (same as V233)
    -- =========================================================================
    ...
    INSERT INTO budget_items (id, budget_id, sequence, category, name, planned_amount, is_section, item_type, discipline_mark,
        ...
        (bsec_eo, bud_id, 1, 'MATERIALS', 'Раздел ЭО — Электроосвещение',        0, TRUE, 'MATERIALS', 'ЭО', NOW(), FALSE),
        (bsec_ov, bud_id, 2, 'MATERIALS', 'Раздел ОВ — Отопление и вентиляция',   0, TRUE, 'MATERIALS', 'ОВ', NOW(), FALSE),
        (bsec_kr, bud_id, 3, 'MATERIALS', 'Раздел КР — Конструктивные решения',    0, TRUE, 'MATERIALS', 'КР', NOW(), FALSE),
        (bsec_vk, bud_id, 4, 'MATERIALS', 'Раздел ВК — Водоснабжение',             0, TRUE, 'MATERIALS', 'ВК', NOW(), FALSE);

    -- ЭО Budget items (materials) — doc_status=PLANNED, will be recalculated in Phase 3
    INSERT INTO budget_items (id, budget_id, parent_id, sequence, category, name, planned_amount, item_type, is_section, quantity, unit, cost_price, sale_price, estimate_price, customer_price, coefficient, discipline_mark, doc_status,
        created_at, deleted) VALUES
        (bi_eo_cable1, bud_id, bsec_eo, 1, 'MATERIALS', 'Кабель ВВГнг-LS 3×2.5',        1440000,  'MATERIALS', FALSE, 12000, 'м',   115.00, 120.00, 130.00, 125.00, 1.04, 'ЭО', 'PLANNED', NOW(), FALSE),
        (bi_eo_cable2, bud_id, bsec_eo, 2, 'MATERIALS', 'Кабель ВВГнг-LS 5×4.0',         840000,  'MATERIALS', FALSE, 3500,  'м',   230.00, 240.00, 260.00, 250.00, 1.04, 'ЭО', 'PLANNED', NOW(), FALSE),
        ...
        (bi_eo_panel,  bud_id, bsec_eo, 5, 'EQUIPMENT', 'Щит ЩР-36 встраиваемый',         576000,  'MATERIALS', FALSE, 24,    'шт',  22000,  24000,  26000,  25000,  1.09, 'ЭО', 'PLANNED', NOW(), FALSE),
    ...
    INSERT INTO budget_items (id, budget_id, parent_id, sequence, category, name, planned_amount, item_type, is_section, quantity, unit, cost_price, sale_price, estimate_price, customer_price, coefficient, discipline_mark, doc_status,
        created_at, deleted) VALUES
        (bi_ov_pipe1, bud_id, bsec_ov, 1, 'MATERIALS', 'Труба стальная Ø 76×3.5 мм',     2160000, 'MATERIALS', FALSE, 1800, 'м',   1100.00, 1200.00, 1300.00, 1250.00, 1.09, 'ОВ', 'PLANNED', NOW(), FALSE),
        ...
        (bi_ov_radia, bud_id, bsec_ov, 3, 'EQUIPMENT', 'Радиатор биметаллический 500/10',  5760000, 'MATERIALS', FALSE, 720,  'шт',  7500.00, 8000.00, 8800.00, 8500.00, 1.07, 'ОВ', 'PLANNED', NOW(), FALSE),
        (bi_ov_boil,  bud_id, bsec_ov, 4, 'EQUIPMENT', 'Котёл газовый настенный 24 кВт',    360000, 'MATERIALS', FALSE, 3,    'шт',  110000,  120000,  135000,  130000,  1.09, 'ОВ', 'PLANNED', NOW(), FALSE),
    ...
    INSERT INTO budget_items (id, budget_id, parent_id, sequence, category, name, planned_amount, item_type, is_section, quantity, unit, cost_price, sale_price, estimate_price, customer_price, coefficient, discipline_mark, doc_status,
        created_at, deleted) VALUES
        (bi_kr_conc,  bud_id, bsec_kr, 1, 'MATERIALS', 'Бетон В25 (М350) с доставкой', 33600000, 'MATERIALS', FALSE, 4200, 'м³',  7500.00, 8000.00, 8800.00, 8500.00, 1.07, 'КР', 'PLANNED', NOW(), FALSE),
        (bi_kr_rebar, bud_id, bsec_kr, 2, 'MATERIALS', 'Арматура А500С Ø 12 мм',       11100000, 'MATERIALS', FALSE, 185,  'т',   56000,   60000,   66000,   64000,   1.07, 'КР', 'PLANNED', NOW(), FALSE),
        (bi_kr_form,  bud_id, bsec_kr, 3, 'EQUIPMENT', 'Опалубка щитовая PERI TRIO',      3600000, 'MATERIALS', FALSE, 1200, 'м²',  2800.00, 3000.00, 3300.00, 3200.00, 1.07, 'КР', 'PLANNED', NOW(), FALSE),
    ...
    INSERT INTO budget_items (id, budget_id, parent_id, sequence, category, name, planned_amount, item_type, is_section, quantity, unit, cost_price, sale_price, estimate_price, customer_price, coefficient, discipline_mark, doc_status,
        created_at, deleted) VALUES
        (bi_kr_work1, bud_id, bsec_kr, 6, 'LABOR', 'Бетонирование фундаментов и перекрытий', 12600000, 'WORKS', FALSE, 4200, 'м³',  2800.00, 3000.00, 3300.00, 3200.00, 1.07, 'КР', 'PLANNED', NOW(), FALSE),
    ...
    INSERT INTO budget_items (id, budget_id, parent_id, sequence, category, name, planned_amount, item_type, is_section, quantity, unit, cost_price, sale_price, estimate_price, customer_price, coefficient, discipline_mark, doc_status,
        created_at, deleted) VALUES
        (bi_vk_pipe1, bud_id, bsec_vk, 1, 'MATERIALS', 'Труба ПЭ 100 SDR 17 Ø 110 мм', 1500000, 'MATERIALS', FALSE, 1500, 'м',   950.00, 1000.00, 1100.00, 1050.00, 1.05, 'ВК', 'PLANNED', NOW(), FALSE),
        ...
        (bi_vk_pump,  bud_id, bsec_vk, 4, 'EQUIPMENT', 'Насосная станция водоснабжения',   900000, 'MATERIALS', FALSE, 3,    'компл',280000, 300000,  330000,  320000,  1.07, 'ВК', 'PLANNED', NOW(), FALSE),
    ...
    -- 5b. CONTRACT TYPES
    INSERT INTO contract_types (id, code, name, active, sequence) VALUES
        (ct_general,    'GENERAL',    'Генеральный подряд',  true, 1),
        (ct_subcontract,'SUBCONTRACT','Субподряд',           true, 2),
        (ct_supply,     'SUPPLY',     'Поставка материалов', true, 3),
        (ct_design,     'DESIGN',     'Проектирование',      true, 4),
        (ct_service,    'SERVICE',    'Услуги',              true, 5);

    -- 6. CONTRACTS
    INSERT INTO contracts (id, name, number, contract_date, partner_name, project_id, organization_id, status, amount, vat_rate, vat_amount, total_with_vat, planned_start_date, planned_end_date, contract_direction, type_id, notes,
        created_at, deleted) VALUES
        (ctr_client, 'Генеральный подряд ЖК "Солнечный Парк"', 'ДОГ-2025-001', '2025-02-15',
         ...
         '2025-03-01', '2026-12-31', 'CLIENT', ct_general,
        ...
        (ctr_sub_eo, 'Субподряд ЭО — ООО "ЭлектроМонтаж"', 'ДОГ-2025-002', '2025-03-10',
         ...
         '2025-04-01', '2026-06-30', 'CONTRACTOR', ct_subcontract,
        ...
        (ctr_sub_ov, 'Субподряд ОВ — ООО "ТеплоСтрой"', 'ДОГ-2025-003', '2025-03-15',
         ...
         '2025-04-15', '2026-09-30', 'CONTRACTOR', ct_subcontract,
        ...
        (ctr_sub_kr, 'Субподряд КР — ООО "МонолитСтрой"', 'ДОГ-2025-004', '2025-03-01',
         ...
         '2025-03-15', '2026-10-31', 'CONTRACTOR', ct_subcontract,
        ...
        (ctr_sup_cable, 'Поставка кабельной продукции — ООО "КабельТорг"', 'ДОГ-2025-005', '2025-03-20',
         ...
         '2025-04-01', '2025-09-30', 'CONTRACTOR', ct_supply,
        ...
        (ctr_sup_pipe, 'Поставка трубной продукции — ООО "ТрубоСнаб"', 'ДОГ-2025-006', '2025-03-25',
         ...
         '2025-04-15', '2025-12-31', 'CONTRACTOR', ct_supply,
        ...
        (ctr_sup_conc, 'Поставка бетона — ООО "БетонПром"', 'ДОГ-2025-007', '2025-03-05',
         ...
         '2025-03-20', '2026-08-31', 'CONTRACTOR', ct_supply,
        ...
        (ctr_sub_vk, 'Субподряд ВК — ООО "АкваМонтаж"', 'ДОГ-2025-008', '2025-04-01',
         ...
         '2025-05-01', '2026-08-31', 'CONTRACTOR', ct_subcontract,
    ...
    -- 9. INVOICES (with budget_item_id on lines where applicable)
    INSERT INTO invoices (id, number, invoice_date, due_date, project_id, contract_id, organization_id, partner_name, invoice_type, status, subtotal, vat_rate, vat_amount, total_amount, paid_amount, discipline_mark, matching_status, notes,
        created_at, deleted) VALUES
        -- All invoices: lines are NET (without VAT), subtotal = SUM(lines), total = subtotal * 1.20
        (inv_cable1, 'СЧ-2025-0067', '2025-04-15', '2025-05-15', proj_id, ctr_sup_cable, org_id,
         'ООО "КабельТорг"', 'RECEIVED', 'PAID', 1380000, 20.00, 276000, 1656000, 1656000,
        ...
        (inv_cable2, 'СЧ-2025-0089', '2025-05-20', '2025-06-20', proj_id, ctr_sup_cable, org_id,
         'ООО "ЭлектроПром"', 'RECEIVED', 'PARTIALLY_PAID', 805000, 20.00, 161000, 966000, 480000,
        ...
        (inv_pipe1, 'СЧ-2025-0102', '2025-04-25', '2025-05-25', proj_id, ctr_sup_pipe, org_id,
         'ООО "ТрубоСнаб"', 'RECEIVED', 'PAID', 1980000, 20.00, 396000, 2376000, 2376000,
        ...
        (inv_pipe2, 'СЧ-2025-0145', '2025-05-10', '2025-06-10', proj_id, ctr_sup_pipe, org_id,
         'ООО "ТрубоСнаб"', 'RECEIVED', 'SENT', 1425000, 20.00, 285000, 1710000, 0,
        ...
        (inv_conc1, 'СЧ-2025-0055', '2025-04-05', '2025-05-05', proj_id, ctr_sup_conc, org_id,
         'ООО "БетонПром"', 'RECEIVED', 'PAID', 7500000, 20.00, 1500000, 9000000, 9000000,
    ...
    -- 10. INVOICE LINES (with budget_item_id linked directly!)
    INSERT INTO invoice_lines (id, invoice_id, name, quantity, unit_of_measure, unit_price, amount, budget_item_id,
        created_at, deleted) VALUES
        (il_cable1_1, inv_cable1, 'Кабель ВВГнг-LS 3×2.5 мм², бухта 500м', 8000, 'м', 115.00, 920000,  bi_eo_cable1, NOW(), FALSE),
        (il_cable1_2, inv_cable1, 'Кабель ВВГнг-LS 3×2.5 мм², бухта 200м', 4000, 'м', 115.00, 460000,  bi_eo_cable1, NOW(), FALSE),
        (il_cable2_1, inv_cable2, 'Кабель ВВГнг-LS 5×4.0 мм²',             3500, 'м', 230.00, 805000,  bi_eo_cable2, NOW(), FALSE),
        (il_pipe1_1,  inv_pipe1,  'Труба стальная Ø 76×3.5 мм, 6м',        1200, 'м', 1100.00,1320000, bi_ov_pipe1,  NOW(), FALSE),
        (il_pipe1_2,  inv_pipe1,  'Труба стальная Ø 76×3.5 мм, 6м (2-я)',   600, 'м', 1100.00, 660000, bi_ov_pipe1,  NOW(), FALSE),
        (il_pipe2_1,  inv_pipe2,  'Труба ПЭ 100 SDR 17 Ø 110 мм',          1500, 'м',  950.00,1425000, bi_vk_pipe1,  NOW(), FALSE),
        (il_conc1_1,  inv_conc1,  'Бетон В25 (М350), доставка миксером',    1000, 'м³', 7500.00,7500000,bi_kr_conc,   NOW(), FALSE),
        (il_rebar_1,  inv_rebar,  'Арматура А500С Ø 12 мм, 11.7м',           80, 'т',  56000, 4480000, bi_kr_rebar,  NOW(), FALSE),
        (gen_random_uuid(), inv_rebar, 'Доставка арматуры на объект',              1, 'компл', 120000, 120000, NULL, NOW(), FALSE),
        (il_lumin_1,  inv_lumin,  'Светильник LED встраиваемый 18W',          400, 'шт', 2850.00,1140000,bi_eo_lumin,  NOW(), FALSE),
        (il_radia_1,  inv_radia,  'Радиатор биметаллический Rifar 500/10',   300, 'шт', 7500.00,2250000,bi_ov_radia,  NOW(), FALSE),
        -- Client invoice lines (no budget_item_id)
        (il_cl1_1, inv_client1, 'СМР по разделу КР — фундамент, 1-й этап',  1, 'компл', 20000000, 20000000, NULL, NOW(), FALSE),
        (il_cl1_2, inv_client1, 'Инженерные сети — подготовительные работы',  1, 'компл', 5000000,   5000000, NULL, NOW(), FALSE),
        (il_cl2_1, inv_client2, 'СМР по КС-3 №1 (апрель-июнь 2025)',         1, 'компл', 18000000, 18000000, NULL, NOW(), FALSE);
    ...
    INSERT INTO payments (id, number, payment_date, project_id, contract_id, organization_id, partner_name, payment_type, status, amount, vat_amount, total_amount, purpose, invoice_id,
        created_at, deleted) VALUES
        -- Payments: amount=net, vat_amount=vat, total_amount=gross (matching invoice totals)
        (gen_random_uuid(), 'ПЛТ-2025-001', '2025-04-20', proj_id, ctr_sup_cable, org_id,
         'ООО "КабельТорг"', 'OUTGOING', 'PAID', 1380000, 276000, 1656000,
        ...
        (gen_random_uuid(), 'ПЛТ-2025-002', '2025-05-10', proj_id, ctr_sup_conc, org_id,
         'ООО "БетонПром"', 'OUTGOING', 'PAID', 7500000, 1500000, 9000000,
        ...
        (gen_random_uuid(), 'ПЛТ-2025-004', '2025-05-20', proj_id, ctr_sup_pipe, org_id,
         'ООО "ТрубоСнаб"', 'OUTGOING', 'PAID', 1980000, 396000, 2376000,
    ...
    -- CP Items (works — linked to estimates via trading_coefficient)
    ...
    -- =========================================================================
    -- PHASE 3: LINK FINANCIAL CHAIN (from V234 logic)
    -- =========================================================================

    -- 3.1 SET PRICE SOURCE on budget items
    ...
    -- 3.2 CALCULATE contracted_amount FROM contract_budget_items
    ...
    -- 3.3 CALCULATE invoiced_amount FROM invoice_lines
    UPDATE budget_items bi
    ...
    -- 3.4 CALCULATE paid_amount FROM payments (prorate via invoice lines)
    --   Formula: line_amount * (invoice.paid / invoice.total)
    --   This correctly handles mixed net/gross conventions in line amounts:
    --   - If lines are gross: paid share = line/total * paid (direct proportion)
    --   - If lines are net: paid share = line * (paid/total) (scales to net basis)
    UPDATE budget_items bi
    ...
    -- 3.5 CALCULATE act_signed_amount FROM KS-2 acts
    UPDATE budget_items bi
    ...
    -- 3.6 RECALCULATE SECTION TOTALS (bottom-up, 2 passes)
    --   Only aggregate monetary totals (planned, contracted, invoiced, paid, act_signed).
    --   Per-unit prices (cost_price, sale_price, estimate_price, customer_price) are
    --   NOT aggregated for sections — they're only meaningful at leaf item level.
    -- Pass 1: leaf → immediate parent
    ...
    -- Pass 2: root sections (if there were sub-sections)
    ...
    UPDATE budget_items SET doc_status = 'PAID'
    WHERE deleted = false AND is_section = false
      AND paid_amount > 0
      AND invoiced_amount > 0
    ...
    -- 3.8 UPDATE CONTRACT TOTALS (total_invoiced, total_paid)
    -- total_invoiced = SUM of invoice total_amounts for each contract
    UPDATE contracts c
    SET total_invoiced = COALESCE(sub.sum_invoiced, 0)
    FROM (
        SELECT contract_id, SUM(total_amount) AS sum_invoiced
        FROM invoices
        WHERE deleted = false AND contract_id IS NOT NULL
        GROUP BY contract_id
    ) sub
    WHERE c.id = sub.contract_id AND c.deleted = false;

    -- total_paid = SUM of payment total_amounts for each contract
    UPDATE contracts c
    SET total_paid = COALESCE(sub.sum_paid, 0)
    FROM (
        SELECT contract_id, SUM(total_amount) AS sum_paid
        FROM payments
        WHERE deleted = false AND status = 'PAID' AND contract_id IS NOT NULL
        GROUP BY contract_id
    ) sub
    WHERE c.id = sub.contract_id AND c.deleted = false;

    RAISE NOTICE 'V235: Clean reseed complete!';
    RAISE NOTICE 'Project: ЖК "Солнечный Парк" (ID: %)', proj_id;
    RAISE NOTICE 'All financial chains linked: invoices→budget, price sources, amounts calculated';

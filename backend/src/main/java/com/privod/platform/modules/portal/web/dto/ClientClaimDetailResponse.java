...
public record ClientClaimDetailResponse(
        UUID id,
        UUID organizationId,
        UUID projectId,
        String claimNumber,
        ...
        Instant createdAt,
        Instant updatedAt,
        List<ClaimActivityResponse> activities
) {
    public static ClientClaimDetailResponse fromEntity(ClientClaim claim, List<ClientClaimActivity> activities) {
        List<ClaimActivityResponse> activityResponses = activities.stream()
                .map(ClaimActivityResponse::fromEntity)
                .toList();

        return new ClientClaimDetailResponse(
                claim.getId(),

...
public class EmploymentContractService {
    ...
    public Page<EmploymentContractResponse> listContracts(ContractStatus status, Pageable pageable) {
        if (status != null) {
            return contractRepository.findByStatusAndDeletedFalse(status, pageable)
